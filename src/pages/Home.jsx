...
const Home = () => {
  ...
  const adjustIslandForScreenSize = (width) => {
    let scale = width < 768 ? [0.8, 0.8, 0.8] : [1, 1, 1];
  ...
  const adjustPlaneForScreenSize = (width) => {
    if (width < 768) {
      return [[1.5, 1.5, 1.5], [0, -6.5, -43]];
    }
    return [[3, 3, 3], [0, -4, -4]];
  };

  const [islandScale, islandPosition, islandRotation] =
    adjustIslandForScreenSize(windowWidth);

  const [planeScale, planePosition] =
    adjustPlaneForScreenSize(windowWidth);
  ...
  return (
    <section className="w-full h-screen relative">
      ...
      <Canvas
        dpr={[1, 1.5]} // reduce resolution for mobile
        performance={{ min: 0.2 }} // performance boost
